...
- [ ] `EnterWorktree` で `rescript-v12-only` worktree を作成

## Phase 2: 実装（worktree 内）

design.md §3 に従い `@rescript/core` の v12 互換最低バージョンを確定:

- [ ] `npm view @rescript/core versions` で利用可能バージョン取得
- [ ] 各バージョンの `peerDependencies.rescript` を確認し v12 互換最低版を抽出（取得失敗時は `@rescript/core >= 1.6.0` 暫定）

design.md §2 に従いファイルを更新:

- [ ] **commit 1**: `docs/product-requirements.md` を v12-only に更新（§2.3 の 6 行 + §10 残課題追記）→ コミット `📝 Drop ReScript 11 from PRD: align dependency policy and roadmap to v12+`
