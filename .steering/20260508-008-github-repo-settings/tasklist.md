...
- [ ] §2.1 Topics 設定（7 値）
- [ ] §2.2 Homepage URL 設定（`https://nagatatz.github.io/rescript-tauri/`）
...
- [ ] §6.2 集約検証: `gh repo view --json` で settings 一覧を取得し before/after を比較

## Phase 3: コード変更（main 直接、worktree なし）

- [ ] **commit 2**: `.github/ISSUE_TEMPLATE/config.yml` を design §3.2 に従い置換（`{{GITHUB_REPO}}` 解消、Discussions リンクを削除し RFC + GHSA リンクに置換）→ コミット `🔧 Resolve {{GITHUB_REPO}} placeholder in ISSUE_TEMPLATE/config.yml`

## Phase 4: マージ準備

- [ ] **commit 3**: tasklist.md を全 `[x]` 化 + 適用結果（gh API レスポンスの要点）を本書末尾に記録 → コミット `📝 Mark steering 20260508-008 complete (record applied repo settings)`
- [ ] `git push origin main` で全コミットを反映

## Phase 5: ユーザー手動作業（Claude 実行不可、引き継ぎメモ）
...
### After（commit 3 直前に取得して埋める）

```
homepageUrl: <fill>
hasProjectsEnabled: <fill>
repositoryTopics: <fill>
private-vulnerability-reporting: <fill>
vulnerability-alerts: <fill>
actions/permissions.sha_pinning_required: <fill>
