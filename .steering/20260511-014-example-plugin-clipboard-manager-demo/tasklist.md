...
- [ ] T1.3 **コミット**: `✨ Add examples/plugin-clipboard-manager-demo (steering 20260511-014)`

## Phase 2: root Cargo workspace 登録

- [ ] T2.1 `Cargo.toml` members に追加
- [ ] T2.2 **コミット**: `🔧 Register plugin-clipboard-manager-demo in root Cargo workspace`

## Phase 3: ドキュメント・CI・CHANGELOG

- [ ] T3.1 `docs/repository-structure.md` §1 + §3 に追加
- [ ] T3.2 `sphinx-docs/user/plugin-clipboard-manager.md` の "See also" に live demo リンク
- [ ] T3.3 `packages/plugin-clipboard-manager/CHANGELOG.md` の `Added` に live example app 言及（既存 Deferred セクションは確認・整合）
- [ ] T3.4 `.github/workflows/examples-build.yml` に build / cargo check の 2 step 追加
- [ ] T3.5 **コミット**: `📝 Link plugin-clipboard-manager-demo from docs / CI / CHANGELOG`

## Phase 4: 検証

- [ ] T4.1 `git diff main..HEAD --name-only` で範囲確認
- [ ] T4.2 build 再実行で green
