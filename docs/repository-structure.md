...
│   ├── tests-coverage.yml                        # 9 パッケージ matrix で vitest v8 カバレッジ計測（観測フェーズ）
...
CI ジョブ定義の詳細は `docs/functional-design.md` §6 を参照。`.template` 拡張子のファイルは opt-in 用テンプレートで、本リポジトリでは現状未有効化。
